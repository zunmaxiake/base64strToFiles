...
var ergodicFiles = function(files){
    return Promise.all(files.map(function(f){
        return getFileTypeAsync(f).then(function(result){
            ...
            //console.log("asdfassdf11111111111:",{filePath:f,fileName:fileName,fileType:fileType})                        
            return Promise.resolve({filePath:f,fileName:fileName,fileType:fileType});                       
        })
    }))   
}
...
var renamesByObjFiles = function(objFiles){
    ...
    return Promise.all(objFiles.map(function(objFile){
        // console.log("renamesByObjFiles objFiles oldparam:",objFile.filePath)
        // console.log("renamesByObjFiles objFiles newparam:",objFile.filePath + objFile.fileType)
        return fs.renameAsync(objFile.filePath,objFile.filePath + objFile.fileType)
        .then(function(){
            // console.log("renamesByObjFiles11111111111:",{path:objFile.filePath + objFile.fileType,fileName:objFile.fileName,fileType:objFile.fileType})
            // console.log("renamesByObjFiles22222222222:",objFile.filePath)
            // console.log("renamesByObjFiles33333333333:",objFile.fileName)
            // console.log("renamesByObjFiles44444444444:",objFile.fileType)
            // console.log("renamesByObjFiles55555555555:",objFile.filePath + objFile.fileType)
            // console.log("renamesByObjFiles66666666666:",objFile.fileName + objFile.fileType)            
            return {filePath:objFile.filePath + objFile.fileType,fileName:objFile.fileName,fileType:objFile.fileType};
        })
        .catch(function(err){
